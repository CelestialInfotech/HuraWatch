...
import { fetchShows, getTvShowsBatch, TvShow } from "@/lib/show-service"
...
export default function TVShowsPage() {
  ...
  const [displayedMovies, setDisplayedMovies] = useState<TvShow[]>([])
  ...
  useEffect(() => {
    const loadMovies = async () => {
      try {
        ...
        // Load first batch
        const firstBatch = getTvShowsBatch(movies, 0, ITEMS_PER_PAGE)
  ...
  const loadMoreMovies = useCallback(() => {
    ...
    setTimeout(() => {
      const nextBatch = getTvShowsBatch(allMovies, page, ITEMS_PER_PAGE)
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 pt-24 pb-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-4">All Shows</h1>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {filteredMovies.map((movie) => (
                 <Link
                 href={`/show/${slugify(movie.title)}`}
                 onClick={() =>
                   sessionStorage.setItem("selectedMovie", JSON.stringify(movie))
                 }
               >
              {/* <Link key={movie.movie_id} href={`/show/${movie.movie_id}`}> */}
                <div className="relative group overflow-hidden rounded-lg aspect-[2/3]">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-3">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <p className="text-xs text-gray-300">{movie.season}</p>
                      <p className="text-xs text-gray-300">●</p>
                      <p className="text-xs text-gray-300">{movie.episode}</p>
